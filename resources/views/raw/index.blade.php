...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="space-y-3">
            ...
            <button id="btn-gnews-search" class="px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-colors">
                Search
            </button>
        </div>
...
$(document).ready(function() {
    $('#btn-gnews-search').on('click', function() {
        var $btn = $(this);
        ...
        $.ajax({
            url: '{{ route("gnews.search") }}',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                query: $('#gnews-query').val(),
                max: $('#gnews-max').val(),
                lang: $('#gnews-lang').val()
            },
            success: function(data) {
                ...
                $('#gnews-search-result').html(html);
            },
            error: function() {
                $('#gnews-search-result').html('<div class="p-3 rounded-lg text-sm bg-red-50 border border-red-200 text-red-800">Request failed.</div>');
            },
            complete: function() {
