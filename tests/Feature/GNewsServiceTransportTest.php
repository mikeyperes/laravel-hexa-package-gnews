...
final class GNewsServiceTransportTest extends TestCase
{
    protected function setUp(): void
    {
        parent::setUp();
        $this->requireInstalledPackage('hexawebsystems/laravel-hexa-package-gnews', GNewsService::class);
        ...
        DB::table('settings')->insert(['key' => 'gnews_api_key', 'value' => 'fixture-secret']);
    }

    public function test_probe_is_pinned_bounded_and_preserves_its_public_contract(): void
    {
        $requests = [];
        $service = $this->service($requests, $this->response(['articles' => []]));
        $this->assertTrue($service->testApiKey('fixture-secret')['success']);
        $this->assertCount(1, $requests);
        $this->assertStringStartsWith('https://gnews.io/api/v4/', $requests[0]->target->url);
        $this->assertSame(10, $requests[0]->timeoutSeconds);
        $this->assertSame(2 * 1024 * 1024, $requests[0]->maxResponseBytes);
    ...
    {
        $requests = [];
        $service = $this->service($requests, $this->response(['articles' => [['title' => 'Fixture article', 'url' => 'https://news.example.test/a', 'image' => null]], 'totalArticles' => 7]));
        ...
        $this->assertSame('gnews', $result['data']['articles'][0]['source_api']);
        $this->assertArrayHasKey('image', $result['data']['articles'][0]);
        parse_str((string) parse_url($requests[0]->target->url, PHP_URL_QUERY), $query);
        $this->assertSame('fixture-secret', $query['token']);
        $this->assertSame('1', $query['max']);
    ...
    {
        foreach (['not-json', '{"unexpected":true}', str_repeat('x', (2 * 1024 * 1024) + 1)] as $body) {
            $requests = [];
    ...
    private function service(array &$requests, OutboundHttpResponse|\Throwable $response, array $addresses = ['93.184.216.34']): GNewsService
    {
        return new GNewsService(new SafeOutboundHttpClient(
